...
def paths_with_partial_sum(node, target_sum, partial_paths):
  if not node: return []
  result = []
  nextPartial = ListDict({target_sum:[[]]})
  for sums, paths in partial_paths.items():
      for path in paths:
         nextPartial[sums-node.value] = [path+[node.name]]
  result = nextPartial[0]
  for child in [node.left, node.right]:
      result += (paths_with_partial_sum(child, target_sum, nextPartial))    
  return result
